...
wxTwainBase::wxTwainBase(wxTwainClient* client,
                         const wxString& appName, const wxString& appFamily,
                         const wxString& appManufacturer, int versionMajor,
                         int versionMinor, const wxString& versionInfo,
                         int language, int country)
{
    Init(client, appName, appFamily, appManufacturer, versionMajor, versionMinor,
         versionInfo, language, country);
}
...
void
wxTwainBase::Init(wxTwainClient* client,
                  const wxString& appName, const wxString& appFamily,
...
void
wxTwainBase::SetClient(wxTwainClient* client)
{
    m_client = client;
}

wxTwainClient*
wxTwainBase::GetClient()
{
    return m_client;
}
...
TW_STATUS
wxTwainBase::GetStatus()
{
    return m_status;
}

TW_INT16
wxTwainBase::GetReturnCode()
{
    return m_returnCode;
}
...
void
wxTwainBase::CloseSourceManager()
{
    if(Ok() && IsSourceManagerOpen())
    {
        CallDSMEntryProc(&m_appId, NULL, DG_CONTROL,
...
bool
wxTwainBase::IsSourceManagerOpen()
{
    return m_sourceManagerIsOpen;
}

int
wxTwainBase::GetSourceCount()
{
...
const TW_IDENTITY*
wxTwainBase::GetSourceIdentity(int i)
{
...
bool
wxTwainBase::IsSourceOpen()
{
    return m_sourceIsOpen;
}

void
wxTwainBase::SetAppLanguage(int language)
{
    m_appId.Version.Language = language;
}

int
wxTwainBase::GetAppLanguage()
{
    return m_appId.Version.Language;
}

void
wxTwainBase::SetAppCountry(int country)
{
    m_appId.Version.Country = country;
}

int
wxTwainBase::GetAppCountry()
{
    return m_appId.Version.Country;
}

void
wxTwainBase::SetAppVersion(int major, int minor, const wxString& versionInfo)
{
...
void
wxTwainBase::GetAppVersion(int* major, int* minor, wxString* versionInfo)
{
    if(major)
...
void
wxTwainBase::SetAppManufacturer(const wxString& manufacturer)
{
    SetStringValue(m_appId.Manufacturer, manufacturer, 32);
}

wxString
wxTwainBase::GetAppManufacturer()
{
    return GetStringValue(m_appId.Manufacturer);
}

void
wxTwainBase::SetAppProductFamily(const wxString& family)
{
    SetStringValue(m_appId.ProductFamily, family, 32);
}

wxString
wxTwainBase::GetProductFamily()
{
    return GetStringValue(m_appId.ProductFamily);
}

void
wxTwainBase::SetAppProductName(const wxString& name)
{
    SetStringValue(m_appId.ProductName, name, 32);
}

wxString
wxTwainBase::GetAppProductName()
{
    return GetStringValue(m_appId.ProductName);
}

bool
wxTwainBase::SelectDefaultSource()
{
...
bool
wxTwainBase::UserSelectSource(const wxString& defSource)
{
  memset(&m_sourceId, 0, sizeof(m_sourceId));
...
bool
wxTwainBase::SelectSource(const wxString& source)
{
    m_sourceSelected = FALSE;
    if(IsSourceManagerOpen())
    {
        ...
        for(i = 0; i < m_sources.GetCount(); i++)
...
bool
wxTwainBase::IsSourceSelected()
{
  return m_sourceSelected;
}

bool
wxTwainBase::CallDSMEntryProc(pTW_IDENTITY origin, pTW_IDENTITY dest,
...
bool
wxTwainBase::DoMessageLoop(bool showUI)
{
    return TRUE;
}
...
bool
wxTwainBase::IsSourceEnabled()
{
  return m_sourceEnabled;
}

void
wxTwainBase::GetSourceVersion(int* major, int *minor, wxString* versionInfo)
{
    if(major)
...
wxString
wxTwainBase::GetSourceProductName()
{
    return GetStringValue(m_sourceId.ProductName);
}

wxString
wxTwainBase::GetSourceProductFamily()
{
    return GetStringValue(m_sourceId.ProductFamily);
}

wxString
wxTwainBase::GetSourceManufacturer()
{
    return GetStringValue(m_sourceId.Manufacturer);
}

void
wxTwainBase::SetStringValue(char *str, const wxString& value, int maxLen)
{
#ifdef __WXMSW__
...
wxString
wxTwainBase::GetStringValue(const char *str)
{
#ifdef __WXMSW__
  return wxString(str);
#else
...
bool
wxTwainBase::SetImageCount(int count)
{
...
bool
wxTwainBase::GetImageInfo(TW_IMAGEINFO& info)
{
    if(IsSourceEnabled())
...
bool
wxTwainBase::GetAvailableSources()
{
    //
...
void
wxTwainBase::DeleteAvailableSources()
{
    size_t i;
